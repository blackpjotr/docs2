---
title: 'Tutorial 13: Anonymous Message Board'
hide_title: true
sidebar_label: 'Tutorial 13: Anonymous Message Board'
...
# Tutorial 13: Anonymous Message Board

This tutorial shows you how to put building zkApp ideas into practice as you walk through designing and implementing of a semi-anonymous messaging protocol.
...
1. Change to the project directory:
  ...
  Each time you make updates, then build or deploy, the TypeScript code is compiled into JavaScript in the `build` directory. See the included [README](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/13-anonymous-message-board/README.md) file with usage instructions.
...
1. Copy the entire contents of the [message.ts](https://github.com/o1-labs/docs2/blob/main/examples/zkapps/13-anonymous-message-board/src/message.ts) example file into your local `message-board/src/message.ts` file.
...
Every Mina smart contract includes 32 on-chain state variables that each store almost 256 bits of information. In more complex smart contracts, these state variables can store commitments to off-chain storage (for example, commitments for the hash of a file, the root of a Merkle tree, and so on).
...
Start with `super.init()`. It resets every state field to its default, and
without it o1js warns that the zkApp will lack the `provedState === true`
status — the status that certifies the state was produced by proofs rather than
set by the developer. Your own values go after it.
...
The tutorial above builds the contract. To see it work end to end, the example
in this repository deploys it to a local blockchain and posts to it — no
network, no faucet and no funded accounts:

```sh
cd examples/zkapps/13-anonymous-message-board
npm install
npm start
```

```
deploying the message board...
approved posters:
  user1: B62qkCkKQ5vSRkTQyEEj22vnHH4hVKfp4pc3QYd4GPSv7u6S4aBE4Fx
  ...
Bob publishes a message...       message: 42
SuperBob publishes another...    message: 99
history hash is the chain of both messages: true
Jack tries to publish...         rejected, as it should be
```

The deployment is an ordinary zkApp deployment — the account creation fee, the
verification key, and `init()` running as part of it, exactly as in
[Tutorial 3](deploying-to-a-network):
...
`npm test` runs the same flow as assertions, including the two cases worth
proving rather than describing: the history hash commits to every message in
sequence, and a key the contract does not know is rejected.
