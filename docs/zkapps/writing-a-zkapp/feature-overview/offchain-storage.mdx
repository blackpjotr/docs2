...
In a previous section, we introduced the concept of on-chain Values. Since Mina currently only supports a total of 32 on-chain Field elements, we need to leverage offchain storage to extend that capacity.
...
covered by the o1js stability guarantee. o1js also ships a second experimental
namespace, `Experimental.V2`. Check the import path against the
