...
Token standards ensure the interoperability of applications on Ethereum. These standardisations are agreed upon in ERCs, Ethereum Requests for Comments, such as the fungible token standard [ERC-20](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/).
...
As shown in this [example code](https://github.com/o1-labs/o1js/blob/main/src/lib/mina/v1/token/token-contract.unit-test.ts), you inherit from the `TokenContract` class:
...
For the full signature of every method below, see the
[`TokenContract` API reference](https://docs.o1labs.org/o1js/api-reference/classes/TokenContract)
on the o1Labs docs.
...
Which utilizes the `Approvable` API to send tokens from one account to another.

### Further examples

The o1js repository is the most reliable source of working token code, since
its examples are compiled and tested with the library:

- [`token-contract.ts`](https://github.com/o1-labs/o1js/blob/main/src/lib/mina/v1/token/token-contract.ts) — the `TokenContract` implementation itself.
- [`token-methods.ts`](https://github.com/o1-labs/o1js/blob/main/src/lib/mina/v1/token/token-methods.ts) — the underlying mint, burn and send methods.
- [`token-with-proofs.ts`](https://github.com/o1-labs/o1js/blob/main/src/examples/zkapps/token-with-proofs.ts) — approving a token transfer with a proof rather than a signature.
