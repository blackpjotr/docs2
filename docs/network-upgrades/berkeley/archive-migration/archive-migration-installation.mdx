...
The Berkeley upgrade is complete. The `minaprotocol/mina-archive-migration` Docker images are no longer published on Docker Hub, and the migration tooling is no longer part of the `mina` repository. This page stays as a record of the procedure.
...
The `mina-archive-migration` Docker images are no longer published. Use the Debian packages above.
