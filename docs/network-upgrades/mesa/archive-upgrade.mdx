...
The Mesa preflight network will hard-fork on **2026-04-27 at 14:00 UTC**. Use the stop-slot release **`4.0.0-preflight1-aaaa`** for every command below — it is the required version for both the pre-fork stabilisation period and the post-fork chain. See [Upgrade Steps](./upgrade-steps) for the full operator checklist.
...
```bash
docker run --rm gcr.io/o1labs-192920/mina-archive:4.0.0-preflight1-aaaa-bookworm-mesa cat /etc/mina/archive/upgrade-to-mesa.sql > upgrade-to-mesa.sql
```
...
apt-get install mina-archive-mesa=4.0.0-preflight1-aaaa
