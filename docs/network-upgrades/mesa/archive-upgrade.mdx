...
hide_title: true
description: General guide for upgrading a Mina archive database to the Mesa schema — applies to Mesa Trail, devnet, and mainnet.
keywords:
  - Mesa
  - upgrade
  - archive
...
:::tip Test-network version info

**Mesa Trail** is the current Mesa test network. See [Mesa Trail Network](/network-upgrades/mesa/mesa-trail) for the current build versions and transaction protocol details.

:::

<Tabs groupId="network">
<TabItem value="mesa-trail" label="Mesa Trail" default>

The Mesa Trail archive version is **`4.0.0-rc2-3418329`**, published as a Debian package only — Mesa Trail has no Docker images. See [Mesa Trail Network](./mesa-trail) for the full release matrix.
...
The devnet Mesa archive version is `4.0.0-6965b50`. Check [Mina releases](https://github.com/MinaProtocol/mina/releases?q=mesa) for the latest tag.

```bash
# Substitute this version wherever <MESA_VERSION> appears below
MESA_VERSION=4.0.0-6965b50
```

</TabItem>
<TabItem value="mainnet" label="Mainnet">

The mainnet Mesa archive version is `4.0.0-6850301`.

```bash
# Substitute this version wherever <MESA_VERSION> appears below
MESA_VERSION=4.0.0-6850301
```
...
The upgrade script ships **inside the Mina archive Debian package**, and inside the Docker image where the release publishes one, so the recommended way to get it is straight from the release you install — that copy is guaranteed to match your archive version.
...
**From the Docker image** — for a release that publishes one (devnet and mainnet; Mesa Trail is Debian-only), extract it from the archive image:

```bash
docker run --rm minaprotocol/mina-archive:<MESA_VERSION>-bookworm-mainnet cat /etc/mina/archive/upgrade_to_mesa.sql > upgrade_to_mesa.sql
```
