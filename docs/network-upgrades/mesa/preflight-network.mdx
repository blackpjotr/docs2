...
The Mesa preflight network will hard-fork on **2026-04-27 at 14:00 UTC**. A new stop-slot release, **`4.0.0-preflight1-aaaa`**, is now available and is **mandatory** for every operator running a Mesa node.

Nodes still on the original preflight build (`4.0.0-preflight1-b649c79`) will fall off the network at the stop transaction slot (2026-04-27 10:00 UTC). Upgrade to `4.0.0-preflight1-aaaa` now — the same binary carries your node across the fork automatically.

See [Upgrade Steps](./upgrade-steps) for the full timeline and operator checklist.
...
**Current version (install this):** `4.0.0-preflight1-aaaa` — stop-slot / hard-fork release.
...
- **Mina Daemon:** `gcr.io/o1labs-192920/mina-daemon:4.0.0-preflight1-aaaa-bookworm-mesa`
- **Archive Node:** `gcr.io/o1labs-192920/mina-archive:4.0.0-preflight1-aaaa-bookworm-mesa`
- **Rosetta:** `gcr.io/o1labs-192920/mina-rosetta:4.0.0-preflight1-aaaa-bookworm-mesa`
...
Available packages:
- `mina-mesa` (version `4.0.0-preflight1-aaaa`)
- `mina-archive-mesa` (version `4.0.0-preflight1-aaaa`)
- `mina-rosetta-mesa` (version `4.0.0-preflight1-aaaa`)
...
docker run --name mina-mesa-preflight -d \
  -p 8302:8302 \
  --restart=always \
  gcr.io/o1labs-192920/mina-daemon:4.0.0-preflight1-aaaa-bookworm-mesa \
  daemon \
  --peer-list-url https://storage.googleapis.com/o1labs-gitops-infrastructure/mina-mesa-network/mina-mesa-network-seeds.txt
```
...
sudo apt-get update
sudo apt-get install -y mina-mesa=4.0.0-preflight1-aaaa
```
...
sudo apt-get update
sudo apt-get install -y mina-archive-mesa=4.0.0-preflight1-aaaa
...
docker run --name mina-archive-mesa -d \
  -p 3086:3086 \
  --restart=always \
  gcr.io/o1labs-192920/mina-archive:4.0.0-preflight1-aaaa-bookworm-mesa \
  mina-archive run \
...
sudo apt-get update
sudo apt-get install -y mina-rosetta-mesa=4.0.0-preflight1-aaaa
...
docker run --name mina-rosetta-mesa -d \
  -p 3087:3087 \
  --restart=always \
  gcr.io/o1labs-192920/mina-rosetta:4.0.0-preflight1-aaaa-bookworm-mesa \
  --port 3087 \
