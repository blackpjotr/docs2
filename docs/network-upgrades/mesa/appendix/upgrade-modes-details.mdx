...
|---|---|---|
| **Pre-fork binary** | `/usr/lib/mina/berkeley/mina` | Runs the current chain up to the stop-network-slot |
| **Post-fork binary** | `/usr/lib/mina/mesa/mina` | Runs the Mesa chain after the fork |
...
|---|---|---|
| Pre-fork (Berkeley) | Before the fork (or to query pre-fork state) | `/usr/lib/mina/berkeley/mina` |
| Post-fork (Mesa) | After the fork | `/usr/lib/mina/mesa/mina` |
...
Invoke the binaries by their full paths — there is no `mina-mesa` or `mina-berkeley` command on `PATH`.
...
Operator-facing troubleshooting (how to tell which binary is active, dispatcher debug mode, when to invoke the pre-fork vs post-fork binary by its full path) has moved to its own page: [Troubleshooting](/network-upgrades/mesa/troubleshooting).
