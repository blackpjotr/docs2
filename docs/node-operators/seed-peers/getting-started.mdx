...
Thirdly, you'll want to pregenerate your libp2p keypair -- this is used to identify you on the gossip network. See [Generating a libp2p Key Pair](/node-operators/seed-peers/generating-a-libp2p-keypair) for instructions.

Then run your daemon with the `--libp2p-keypair <path-to-the-key-file>` and `--seed` flags:
...
Follow along as normal with systemctl to start the mina service:
...
Run the image with your config and keys mounted into the container:
