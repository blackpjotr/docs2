...
[Rosetta API](https://www.rosetta-api.org/) is an open-source specification and set of tools that make deploying and interacting with blockchains quick and easy. Mina's Rosetta implementation provides a standard interface for:
...
docker run -it --rm --name rosetta \
  ...
  minaprotocol/mina-rosetta:3.3.1-7b34378-focal-mainnet
```
...
docker run -it --rm --name rosetta \
  ...
  minaprotocol/mina-rosetta:3.2.0-97ad487-bullseye-devnet
```

:::caution
Do **not** set `MINA_GENESIS_LEDGER_URL` unless you have a specific reason to override the genesis ledger baked into the image. Providing an incorrect or outdated URL will cause the daemon to crash with a "Tarball hash mismatch" error. The image already contains the correct genesis configuration for its target network.
:::

The node will take 20 minutes to 1 hour to sync. You can check sync status with:
...
The Mina repository includes a ready-to-use Docker Compose configuration:

```bash
# Clone the compose files
curl -O https://raw.githubusercontent.com/MinaProtocol/mina/develop/src/app/rosetta/docker-compose/docker-compose.yml
curl -O https://raw.githubusercontent.com/MinaProtocol/mina/develop/src/app/rosetta/docker-compose/example.env

# Copy and edit the environment file
cp example.env .env
# Edit .env to set your image tags and configuration
```

The compose setup runs four services:
- **postgres** — PostgreSQL database
- **archive** — `mina-archive` connected to PostgreSQL
- **daemon** — Mina node connected to the archive
- **rosetta** — Rosetta API in standalone mode, querying the daemon and database

```bash
docker compose up -d
```

See the [docker-compose directory](https://github.com/MinaProtocol/mina/tree/develop/src/app/rosetta/docker-compose) in the Mina repository for the full configuration and environment file reference.
...
echo "deb [trusted=yes] http://packages.o1test.net/ focal stable" | sudo tee /etc/apt/sources.list.d/mina.list
sudo apt-get update
...
| Network | Image | Notes |
|---|---|---|
| **Mainnet** | `minaprotocol/mina-rosetta:3.3.1-7b34378-focal-mainnet` | Latest stable, amd64 |
| **Mainnet (bookworm)** | `minaprotocol/mina-rosetta:3.3.1-7b34378-bookworm-mainnet` | Also available for arm64 |
| **Devnet** | `minaprotocol/mina-rosetta:3.2.0-97ad487-bullseye-devnet` | Latest devnet |
...
For most users, the official images are sufficient. If you need to build a custom image, see the [Rosetta README](https://github.com/MinaProtocol/mina/blob/develop/src/app/rosetta/README.md) in the Mina repository for build instructions.
