...
docker run -it --rm --name rosetta \
  ...
  -p 8302:8302 -p 3085:3085 -p 127.0.0.1:3086:3086 -p 3087:3087 \
  minaprotocol/mina-rosetta:4.0.0-6850301-noble-mainnet
```
...
docker run -it --rm --name rosetta \
  ...
  minaprotocol/mina-rosetta:4.0.0-6965b50-bookworm-devnet
```
...
For production deployments, a complete Docker Compose configuration with all four services (PostgreSQL, archive node, Mina daemon, and Rosetta API) is available in the [Rosetta Docker Compose guide](/node-operators/rosetta/docker-compose).
...
| Network | Image | Notes |
|---|---|---|
| **Mainnet (noble)** | `minaprotocol/mina-rosetta:4.0.0-6850301-noble-mainnet` | amd64 |
| **Mainnet (bookworm)** | `minaprotocol/mina-rosetta:4.0.0-6850301-bookworm-mainnet` | Also available for arm64 |
| **Devnet (bookworm)** | `minaprotocol/mina-rosetta:4.0.0-6965b50-bookworm-devnet` | Latest devnet |
