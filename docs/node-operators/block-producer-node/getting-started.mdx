...
Get started setting up a Mina node, learn how to [generate a key pair](/node-operators/validator-node/generating-a-keypair) for use with the network, and install the `mina daemon`.
...
Use the Docker installation instructions for your platform on [Get Docker](https://docs.docker.com/get-docker/). After Docker is installed, follow the [Connect to the Mina Network](/node-operators/validator-node/connecting-to-the-network#start-a-mina-node-with-auto-restart-flows) instructions.
