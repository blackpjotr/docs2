...
1. Install the archive node package.
   ...
   - Ubuntu/Debian:
     ...
     ```
     sudo apt-get install mina-archive=4.0.0-6850301
     ```

   - Docker:

     ```
     minaprotocol/mina-archive:4.0.0-6850301-bullseye-mainnet
     ```
...
:::danger Do not expose the archive server port

The archive server port (`--server-port`, 3086 in these examples) has no authentication. Anyone who can reach it can write blocks into your archive database.

Only the daemon or daemons feeding this archive should be able to reach the port. Keep the daemon and archive on localhost, a private network, a Docker network, or inside one Kubernetes cluster, and never expose the port to the public internet. On a shared Kubernetes cluster, add a NetworkPolicy so only the daemon pods can reach the archive.
...
1. Start the mina daemon and connect it to the archive process that you started on port 3086:
   ...
   To connect to an archive process on another machine, specify `<ipaddress:portnumber>`, for example `10.0.0.5:3086`. Both machines must be on a trusted private network.
...
2. Pull the archive node image from Docker Hub.
   ...
   ```sh
   docker pull minaprotocol/mina-archive:4.0.0-6850301-bullseye-mainnet
   ```
...
4. Start the postgres container and expose its networking to other containers.
...
1. Install Docker and Docker Compose on your machine. For more information, see [Docker](https://docs.docker.com/get-docker/) and [Docker Compose](https://docs.docker.com/compose/install/).
...
3. Create a local directory to store the archive node data.
...
5. Start the archive node.

   ```sh
   docker compose up
   ```

6. Start the mina daemon and connect it to the archive process that you started on port 3086:
