...
- Run as a systemd service (see [the docs](./block-producer-node/connecting-to-the-network#ubuntu-1804--debian-9-1)).
- Run with Docker (see [the docs](./block-producer-node/connecting-to-the-network#docker)).
...
See the requirements for the current network [here](./block-producer-node/getting-started). Some lower-tier VPS providers may have an issue producing blocks within the slot time. If you continue to experience missing blocks, you may wish to try on more powerful hardware.
...
This is an optional file to pass [configuration options](./mina-cli-reference#mina-daemon) to the daemon (rather than passing via the command line). The daemon looks for it by default, which results in an error if you do not include it. Just ignore this error (unless you are trying to include it) as it is not fatal, and the daemon will start as per normal.
...
You can cancel pending transactions. See [Mina Client](./mina-cli-reference#mina-client) or wallet instructions.
...
The methods to start a node [in the documentation](./block-producer-node/connecting-to-the-network) will run a block producer by default.
...
The `~/.mina-config` directory contains the Mina logs, see [Logging](logging). This directory contains the following:
