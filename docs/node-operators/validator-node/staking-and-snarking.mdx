...
In addition to block production, you can earn rewards by running a SNARK worker that generates [zk-SNARKs](/glossary#zk-snark) to compress transaction data on the network.

For details on running SNARK workers, including hardware requirements, setup commands, and coordinator configuration, see the [SNARK Workers Getting Started](/node-operators/snark-workers/getting-started) guide. For common questions about SNARK pricing and fees, see the [FAQ](/node-operators/faq#snarks-and-snark-workers).
