...
title: Mina Signer
hide_title: true
description: A NodeJS/Browser-compatible JavaScript library for signing transactions, generating keys, and integrating with o1js and Rosetta in the Mina Protocol.
keywords:
  - mina signer
  - sign
  - signature
...
For detailed Rosetta usage including the offline signer CLI tool and `signRosettaTransaction`, see the [Rosetta Offline Signer](/exchange-operators/rosetta/samples/using-signer) documentation.
...
In Mina Signer, the Field type is a BigInt (while in o1js they are a separate data structure), so you must convert the fields from o1js to BigInts before signing/verifying them.
