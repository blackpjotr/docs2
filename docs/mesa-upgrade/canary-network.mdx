---
title: Connect to Mesa Canary Network
sidebar_label: Canary Network
description: Instructions for connecting to the Mesa upgrade canary network for testing and validation.
keywords:
  - Mesa upgrade
  - canary network
  - testing network
...
# Connect to Mesa Canary Network

The Mesa canary network is a testing environment for validating the Mesa upgrade before deployment to devnet and mainnet. This network allows node operators and developers to test their infrastructure and applications with the new Mesa release.

:::warning Canary Network Notice

The canary network is intended for testing purposes only. This network may experience instability, breaking changes, and unexpected behavior. Data on this network should not be considered persistent or reliable.

:::

## Available Build Versions

The Mesa canary network uses the following build versions:

**Version:** `4.0.0-canary1-b649c79`

### Docker Images

Docker images are available for both `amd64` and `arm64` architectures:

- **Mina Daemon:** `gcr.io/o1labs-192920/mina-daemon:4.0.0-canary1-b649c79-bookworm-mesa`
- **Archive Node:** `gcr.io/o1labs-192920/mina-archive-mesa:4.0.0-canary1-b649c79-bookworm-mesa`
- **Rosetta:** `gcr.io/o1labs-192920/mina-rosetta-mesa:4.0.0-canary1-b649c79-bookworm-mesa`
...
Available packages:
- `mina-mesa` (version `4.0.0-canary1-b649c79`)
- `mina-archive-mesa` (version `4.0.0-canary1-b649c79`)
- `mina-rosetta-mesa` (version `4.0.0-canary1-b649c79`)
...
To connect your node to the Mesa canary network, you must use the canary network seed peer list.
...
```bash
docker run --name mina-mesa-canary -d \
  -p 8302:8302 \
  --restart=always \
  gcr.io/o1labs-192920/mina-daemon:4.0.0-canary1-b649c79-bookworm-mesa \
  daemon \
  --peer-list-url https://storage.googleapis.com/o1labs-gitops-infrastructure/mina-mesa-network/mina-mesa-network-seeds.txt
```
...
# Step 3: Add the unstable repository with canary channel
echo "deb https://unstable.apt.packages.minaprotocol.com $(lsb_release -cs) canary" | \
  sudo tee /etc/apt/sources.list.d/mina-canary.list

# Step 4: Update and install
sudo apt-get update
sudo apt-get install -y mina-mesa=4.0.0-canary1-b649c79
```
...
sudo apt-get update
sudo apt-get install -y mina-archive-mesa=4.0.0-canary1-b649c79
...
docker run --name mina-archive-mesa -d \
  -p 3086:3086 \
  --restart=always \
  gcr.io/o1labs-192920/mina-archive-mesa:4.0.0-canary1-b649c79-bookworm-mesa \
  mina-archive run \
...
sudo apt-get update
sudo apt-get install -y mina-rosetta-mesa=4.0.0-canary1-b649c79
...
docker run --name mina-rosetta-mesa -d \
  -p 3087:3087 \
  --restart=always \
  gcr.io/o1labs-192920/mina-rosetta-mesa:4.0.0-canary1-b649c79-bookworm-mesa \
  --port 3087 \
...
In addition to the canary release builds, nightly builds are also available for testing the latest changes.

**Repository:** `nightly.apt.packages.minaprotocol.com`

Nightly builds are available for both Debian packages and Docker images. These builds represent the latest development state and may be even more unstable than canary releases.
...
After starting your node, verify connectivity to the canary network:

### Check Node Status

```bash
# For Docker
docker exec -it mina-mesa-canary mina client status
...
```bash
# For Docker
docker logs -f mina-mesa-canary
...
If you encounter issues or have feedback about the Mesa canary network:
...
- Test your applications and infrastructure against the canary network
