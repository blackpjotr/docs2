...
```bash
docker run --rm gcr.io/o1labs-192920/mina-archive-mesa:4.0.0-canary1-b649c79-bookworm-mesa cat /etc/mina/archive/upgrade-to-mesa.sql > upgrade-to-mesa.sql
```

```bash
# Setup the Mina repository and install the archive package
# See canary-network.mdx for detailed repository setup instructions

apt-get install mina-archive-mesa=4.0.0-canary1-b649c79
